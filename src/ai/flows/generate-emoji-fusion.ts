...
/**
 * @fileOverview Generates a new emoji by fusing two existing emojis.
 *
...
const GenerateEmojiFusionInputSchema = z.object({
  emoji1: z.string().describe('The first emoji to fuse.'),
  emoji2: z.string().describe('The second emoji to fuse.'),
});
export type GenerateEmojiFusionInput = z.infer<
  typeof GenerateEmojiFusionInputSchema
>;
...
export type GenerateEmojiFusionOutput = z.infer<
  typeof GenerateEmojiFusionOutputSchema
>;

export async function generateEmojiFusion(
  input: GenerateEmojiFusionInput
): Promise<GenerateEmojiFusionOutput> {
...
const fusionPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI that can fuse two emojis together to create a new one.
